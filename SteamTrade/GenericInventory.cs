...
namespace SteamTrade
{
    ...
    public class GenericInventory
    {
        ...
        public Inventory.Item GetItem(int appId, long contextId, ulong id, bool isCurrency)
        {
            ...
            {
                AddForeignInventory(UserId, BotId, appId, contextId);
                return GetItem(appId, contextId, id, isCurrency);
            }
        }
        ...
        private string RetryWebRequest(string url, SteamID botId)
        {
            for (int i = 0; i < WEB_REQUEST_MAX_RETRIES; i++)
            {
                try
                {
                    return SteamWeb.Fetch(url, "GET", null, Cookies[botId], false, "http://steamcommunity.com");
                }
                catch (Exception ex)
                {
                    Console.WriteLine(ex);
